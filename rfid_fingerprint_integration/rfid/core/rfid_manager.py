...
class RFID_Manager:
    ...
    def write_string(self, start_block: int, text: str, max_length: int = 3000) -> bool:
        ...
        # NEW APPROACH: Use first block of each sector to avoid intra-sector auth issues
        # Start from sector that contains start_block and use first block of subsequent sectors
        available_blocks = []
        
        # Determine starting sector
        start_sector = start_block // 4
        
        print(f"Using cross-sector approach starting from sector {start_sector}")
        
        # Use first data block of each sector (4, 8, 12, 16, 20, etc.)
        for sector_num in range(start_sector, 16):  # MIFARE 1K has 16 sectors
            first_block = sector_num * 4
            
            # Skip sector 0 entirely (contains UID and manufacturer data)
            if sector_num == 0:
                continue
                
            # For the starting sector, use the requested start_block if it's valid
            if sector_num == start_sector and start_block % 4 != 3:  # Not a trailer
                available_blocks.append(start_block)
            else:
                # Use first data block of this sector
                available_blocks.append(first_block)
            
            # Stop when we have enough blocks
        ...
        # Use batch writing approach to avoid cache invalidation issues
        print(f"Using batch write approach for {chunks_needed} blocks...")
        
        import subprocess
        
        # Read entire card once
        card_data = self.read_card_raw()
        if not card_data:
            print("Failed to read card for batch write")
            return False
            
        # Modify all blocks in memory
        card_data = bytearray(card_data)
        for i in range(chunks_needed):
            block_num = available_blocks[i]
            chunk_data = data[i * 16:(i + 1) * 16]
            start = block_num * 16
            card_data[start:start + 16] = chunk_data
            print(f"Prepared chunk {i+1}/{chunks_needed} for block {block_num}")
        
        # Write entire card once
        write_file = os.path.join(self.temp_dir, "rfid_batch_write.mfd")
        try:
            with open(write_file, 'wb') as f:
                f.write(card_data)
            
            print("Writing batched changes to card...")
            result = subprocess.run([
                'nfc-mfclassic', 'w', 'A', 'u', write_file
            ], capture_output=True, text=True, timeout=20)
            ...
            if result.returncode == 0:
                print("✅ Batch write successful! Verifying blocks...")
                
                # Invalidate cache and verify all blocks
                if os.path.exists(self.temp_file):
                    os.remove(self.temp_file)
                
                import time
                time.sleep(0.2)  # Let card settle
                
                # Verify all written blocks
                success_count = 0
                for i in range(chunks_needed):
                    block_num = available_blocks[i]
                    expected_data = data[i * 16:(i + 1) * 16]
                    verification_data = self.read_block(block_num)
                    
                    if verification_data and verification_data == expected_data:
                        success_count += 1
                        print(f"✓ Block {block_num} verification passed")
                    else:
                        print(f"❌ Block {block_num} verification FAILED!")
                        print(f"Expected: {expected_data.hex()}")
                        print(f"Got:      {verification_data.hex() if verification_data else 'None'}")
                        print(f"Batch write succeeded but {chunks_needed - success_count} blocks failed verification")
                        return False
                
                print(f"✅ All {success_count}/{chunks_needed} blocks verified successfully")
                print(f"String written across blocks: {available_blocks[0]} to {available_blocks[chunks_needed-1]}")
                return True
            else:
                print(f"❌ Batch write failed: {result.stderr}")
                return False
    ...
    def read_string(self, start_block: int, max_length: int = 3000) -> Optional[str]:
        ...
        try:
            ...
            # Calculate which blocks were used for writing (cross-sector approach)
            card_info = self.get_card_info()
            max_blocks = 256 if card_info.get('type') == 'MIFARE Classic 4K' else 64
            
            # Calculate needed blocks using same logic as write_string
            bytes_for_string = len(raw_bytes) + (char_count - len(raw_bytes.decode("utf-8", errors="ignore")))
            bytes_needed = bytes_for_string
            chunks_needed = (bytes_needed + 15) // 16  # Round up
            
            # Get the block sequence that was used for writing (same logic as write_string)
            start_sector = start_block // 4
            read_blocks = [start_block]  # We already have the first block
            
            for sector_num in range(start_sector + 1, 16):  # Continue from next sector
                if sector_num == 0:  # Skip sector 0
                    continue
                first_block = sector_num * 4
                read_blocks.append(first_block)
                if len(read_blocks) >= chunks_needed:
                    break
            
            print(f"Cross-sector read sequence: {read_blocks[:chunks_needed]}")
            
            chars_decoded = 0
            blocks_read = 0
            block_index = 1  # Start from second block (we already have first)
            
            while chars_decoded < char_count and block_index < len(read_blocks) and blocks_read < 10:
                # Check timeout
                if time.time() - start_time > timeout_seconds:
                    print(f"Timeout after {timeout_seconds}s while reading string")
                    break
                
                current_block = read_blocks[block_index]
